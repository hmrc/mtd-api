...
class SampleDirectNoCatsControllerSpec
  ...
  trait Test {
    ...
    val controller = new SampleDirectNoCatsController(
  ...
  "handleRequest" should {
    ...
    "return the error as per spec" when {
      ...
      "service errors occur" must {
        def serviceErrors(errorCode: DesErrorCode, mtdError: MtdError, expectedStatus: Int): Unit = {
          s"translate a $errorCode to $mtdError with status $expectedStatus" in new Test {
            ...
            MockSampleService
              .doService(requestData)
              .returns(Future.successful(Left(ResponseWrapper(correlationId, DesErrors.single(errorCode)))))
        ...
        val input = Seq(
          (DesErrorCode("NOT_FOUND"), NotFoundError, NOT_FOUND),
          (DesErrorCode("SERVER_ERROR"), DownstreamError, INTERNAL_SERVER_ERROR),
          (DesErrorCode("SERVICE_UNAVAILABLE"), DownstreamError, INTERNAL_SERVER_ERROR)
        )
